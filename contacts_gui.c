...
static void on_send_button_clicked(GtkButton *btn, gpointer user_data) {
    ...
    send_message(data->db, data->contact_id, msg_text);        // Save locally
    ...
    if (recipient_ip) {
        send_message_over_network(recipient_ip, msg_text, data->contact_id);
        g_free(recipient_ip);
    }
...
/* Open messaging window for given contact id */
static void open_messaging_window(AppWidgets *app, int contact_id) {
    ...
    // Load message history (your implementation)
    load_message_history(app->db, contact_id, history_buffer);
    ...
    g_signal_connect(window, "destroy", G_CALLBACK(on_messaging_window_destroy), send_data);
...
int main(int argc, char *argv[]) {
    ...
    AppWidgets app;
    memset(&app, 0, sizeof(AppWidgets));
    ...
    // Initialize messaging DB (your implementation)
    if (!init_message_db(app.db)) {
