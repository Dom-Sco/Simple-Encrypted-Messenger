...
#define PRIVATE_KEY_FILE "ec_private_key.pem"
#define PUBLIC_KEY_FILE "ec_public_key.pem"
...
static void on_send_button_clicked(GtkButton *btn, gpointer user_data) {
    ...
    if (sqlite3_prepare_v2(data->db, sql, -1, &stmt, NULL) == SQLITE_OK) {
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if (ip) recipient_ip = g_strdup((const char *)ip);
            if (pubkey) recipient_pubkey = g_strdup((const char *)pubkey);
        }
        sqlite3_finalize(stmt);
    }

    if (recipient_ip && recipient_pubkey) {
        // Send over network to recipient IP, but include recipient public key in message metadata
        send_message_over_network(recipient_ip, msg_text, data->self_pubkey, data->contact_id);

        g_free(recipient_ip);
        g_free(recipient_pubkey);
    }
...
int main(int argc, char *argv[]) {
    ...
    app.self_name = g_strdup(self_name);         // If you added self_name to AppWidgets
    ...
    if (sqlite3_prepare_v2(app.db, sql, -1, &stmt, NULL) == SQLITE_OK) {
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if (pk) app.self_pubkey = g_strdup((const char *)pk);
        }
        sqlite3_finalize(stmt);
}
