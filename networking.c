 ...
 */
int send_message_over_network(const char *ip, const char *message, int sender_id, int receiver_id) {
    int sockfd;
    struct sockaddr_in server_addr;
    char buffer[BUFFER_SIZE];

    // Prepare message format: <sender_id>\n<receiver_id>\n<message>\n
...
void *server_thread_func(void *arg) {
    ...
    while (1) {
        ...
        // Expecting format: <sender_contact_id>\n<receiver_contact_id>\n<message>\n
        ...
        *second_newline = '\0';
        int receiver_id = atoi(first_newline + 1);

        const char *msg_text = second_newline + 1;
        char *msg_end = strchr(msg_text, '\n');
        if (msg_end) *msg_end = '\0';

        printf("[server] Received message from %d to %d: %s\n", sender_id, receiver_id, msg_text);

        // Save message to DB (update send_message to accept sender and receiver)
        send_message(global_db, sender_id, receiver_id, msg_text);

        // Prepare notification for UI - notify with sender_id to update chat window
